...
def build_api_kwargs(agent, api_messages: list) -> dict:
    ...
    # ── Clamp max_tokens to prevent physical context limit overflow ──
    try:
        phys_limit = getattr(agent, "_config_physical_context_length", None)
        if phys_limit is None:
            comp = getattr(agent, "context_compressor", None)
            phys_limit = getattr(comp, "physical_context_length", None)
        if phys_limit is None:
            phys_limit = 119500

        from agent.model_metadata import estimate_messages_tokens_rough
        input_tokens = estimate_messages_tokens_rough(api_messages)
        tool_tokens = 0
        if agent.tools:
            tool_tokens = (len(json.dumps(agent.tools, ensure_ascii=False)) + 3) // 4
        
        total_input = input_tokens + tool_tokens
        available_out = phys_limit - total_input - 4096
        
        if available_out > 0:
            _ephemeral_out = getattr(agent, "_ephemeral_max_output_tokens", None)
            if _ephemeral_out is not None and _ephemeral_out > available_out:
                agent._ephemeral_max_output_tokens = max(1024, available_out)
                logger.info(
                    "Clamping ephemeral_max_output_tokens to %d (physical limit %d, input %d)",
                    agent._ephemeral_max_output_tokens, phys_limit, total_input
                )
            
            _resolved_max = agent.max_tokens if agent.max_tokens is not None else 65536
            if _resolved_max > available_out:
                _clamped_val = max(1024, available_out)
                if getattr(agent, "_ephemeral_max_output_tokens", None) is None:
                    agent._ephemeral_max_output_tokens = _clamped_val
                    logger.info(
                        "Clamping max_tokens to %d (physical limit %d, input %d) via ephemeral override",
                        _clamped_val, phys_limit, total_input
                    )
    except Exception as exc:
        logger.warning("Failed to clamp max_tokens to physical limit: %s", exc)
    ...
    try:
        ...
        if hasattr(agent, 'context_compressor') and agent.context_compressor:
            ...
            agent.context_compressor.update_model(
                ...
                physical_context_length=getattr(agent, "_config_physical_context_length", None),
            )
