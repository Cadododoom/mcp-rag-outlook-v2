...
def build_system_prompt(agent: Any, system_message: Optional[str] = None) -> str:
    ...
    # Inject truncation warning header if compaction is active
    if getattr(agent, "context_compressor", None) and (getattr(agent.context_compressor, "compression_count", 0) > 0 or getattr(agent.context_compressor, "_previous_summary", None) is not None):
        warning_header = (
            "=== 🗜️ CONTEXT TRUNCATION WARNING ===\n"
            "Older parts of this conversation history have been truncated and compacted "
            "into a handoff summary to preserve context space. If you need to retrieve "
            "exact details, files, or information from earlier turns, you MUST use your "
            "search/retrieval tools (e.g., retrieve_chat_memory, query_edge_rag, or view_file) "
            "instead of hallucinating or assuming they do not exist.\n"
            "=========================================\n\n"
        )
        joined = warning_header + joined

    # Inject context-cliff warning header if context is nearing limit
    comp = getattr(agent, "context_compressor", None)
    if comp:
        context_max = int(getattr(comp, "context_length", 0) or 0)
        measured_used = int(getattr(comp, "last_prompt_tokens", 0) or 0)
        if context_max > 0 and measured_used > 0 and (measured_used / context_max) >= 0.90:
            cliff_warning = (
                "=== ⚠️ CONTEXT CLIFF WARNING ===\n"
                "Your physical context window is 90%+ full. Crucial system rules and instructions "
                "are at risk of being truncated. You must immediately summarize your active progress "
                "and run the /compress command to rotate the session and clean context space.\n"
                "=================================\n\n"
            )
            joined = cliff_warning + joined
