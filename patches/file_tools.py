...
def _verify_written_file(file_path: str, expected_content: str) -> str | None:
    try:
        if not os.path.exists(file_path):
            return "Filesystem validation error: Written file does not exist on disk."
        with open(file_path, "r", encoding="utf-8", errors="replace") as f:
            disk_content = f.read()
        if disk_content != expected_content:
            return "Filesystem validation error: Written file content on disk does not match expected content."
    except Exception as e:
        return f"Filesystem validation error: Failed to verify file contents on disk: {e}"
    return None
    ...
    try:
        ...
        if _resolved is None:
            ...
            if not result_dict.get("error"):
                verify_err = _verify_written_file(path, content)
                if verify_err:
                    result_dict["error"] = verify_err
            if stale_warning:
        ...
        with file_state.lock_path(_resolved):
            ...
            result_dict = result.to_dict()
            if not result_dict.get("error"):
                verify_err = _verify_written_file(_resolved, content)
                if verify_err:
                    result_dict["error"] = verify_err
