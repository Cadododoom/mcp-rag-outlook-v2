...
class ContextCompressor(ContextEngine):
    ...
    def get_virtual_tokens(self, active_tokens: int) -> int:
        """Query LanceDB memory store to calculate the total virtual context size for the session."""
    ...
    def update_model(
        ...
        physical_context_length: int | None = None,
    ) -> None:
        ...
        if physical_context_length is None:
            self.physical_context_length = context_length
        else:
            self.physical_context_length = physical_context_length
        ...
        self.threshold_tokens = self._compute_threshold_tokens(
            self.physical_context_length, self.threshold_percent, self.max_tokens,
        )
        ...
        self.max_summary_tokens = min(
            int(self.physical_context_length * 0.05), _SUMMARY_TOKENS_CEILING,
        )
    ...
    def __init__(
        ...
        if physical_context_length is None:
            self.physical_context_length = self.context_length
        else:
            self.physical_context_length = physical_context_length
        ...
        self.threshold_tokens = self._compute_threshold_tokens(
            self.physical_context_length, threshold_percent, self.max_tokens,
        )
        ...
        if not quiet_mode:
            logger.info(
                "Context compressor initialized: model=%s context_length=%d physical_context_length=%d "
                "threshold=%d (%.0f%%) target_ratio=%.0f%% tail_budget=%d "
                "provider=%s base_url=%s",
                model, self.context_length, self.physical_context_length, self.threshold_tokens,
...
This compaction should PRIORITISE preserving all information related to the focus topic above. For content related to "{focus_topic}", include full detail — exact values, file paths, command outputs, error messages, and decisions. For content NOT related to the focus topic, summarise more aggressively (brief one-liners or omit if truly irrelevant). The focus topic sections should receive roughly 60-70% of the summary token budget. Even for the focus topic, NEVER preserve API keys, tokens, passwords, or credentials — use [REDACTED]."""
    ...
    def compress(self, messages: List[Dict[str, Any]], current_tokens: int = None, focus_topic: str = None, force: bool = False) -> List[Dict[str, Any]]:
        ...
        # Index compacted turns to LanceDB memory store
        if getattr(self, "_session_id", None) and summary:
            try:
                from agent.lancedb_indexer import index_turns
                details_text = ""
                for msg in turns_to_summarize:
                    role = msg.get("role", "unknown")
                    content = msg.get("content", "")
                    details_text += f"[{role}]: {content}\n\n"
                
                db_path = "/home/theworks/.hermes/data/lancedb_store"
                index_turns(self._session_id, summary, details_text, db_path)
            except Exception as e:
                logger.error("Failed to index compacted turns in LanceDB: %s", e)
