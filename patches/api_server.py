...
class APIServerAdapter(BasePlatformAdapter):
    ...
    async def _handle_session_context(self, request: "web.Request") -> "web.Response":
        """GET /api/session/context - Get active session context status and limits."""
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
        ...
        if not session_id:
            return web.json_response({"error": "No active session found"}, status=400)

        physical_tokens = 0
        if db:
            messages = db.get_messages(session_id)
            char_count = sum(len(m.get("content") or "") for m in messages)
            physical_tokens = int(char_count / 4.0)

        virtual_tokens = physical_tokens
        import os
        db_path = os.getenv("LANCE_DB_PATH") or "/home/theworks/.hermes/data/lancedb_store"
        if os.path.exists(db_path):
            try:
                import lancedb
                ldb = lancedb.connect(db_path)
                if "agent_memories" in ldb.table_names():
                    table = ldb.open_table("agent_memories")
                    res = table.search().where(f'conversationId == "{session_id}"').to_arrow()
                    details_list = res.to_pydict().get("details", [])
                    db_chars = sum(len(d) for d in details_list if d)
                    db_tokens = int(db_chars / 4.2)
                    virtual_tokens += db_tokens
            except Exception as e:
                pass

        physical_context_limit = None
        virtual_context_limit = None
        
        session_override = self._session_model_override_for(session_id)
        if session_override and isinstance(session_override, dict):
            physical_context_limit = session_override.get("physical_context_length")
            virtual_context_limit = session_override.get("context_length")
            
        if (physical_context_limit is None or virtual_context_limit is None):
            model = None
            if session_override and isinstance(session_override, dict):
                model = session_override.get("model")
            if not model:
                try:
            ...
            if model:
                ...
                if route:
                    if physical_context_limit is None:
                        physical_context_limit = route.get("physical_context_length")
                    if virtual_context_limit is None:
                        virtual_context_limit = route.get("context_length")
            ...
            try:
                ...
                if user_config and isinstance(user_config, dict):
                    ...
                    if isinstance(model_cfg, dict):
                        if physical_context_limit is None:
                            physical_context_limit = model_cfg.get("physical_context_length")
                        if virtual_context_limit is None:
                            virtual_context_limit = model_cfg.get("context_length")
            except Exception:
                pass
                
        if physical_context_limit is None:
            physical_context_limit = 120000
        if virtual_context_limit is None:
            virtual_context_limit = 2000000
            
        try:
            physical_context_limit = int(physical_context_limit)
        except Exception:
            physical_context_limit = 120000
        try:
            virtual_context_limit = int(virtual_context_limit)
        except Exception:
            virtual_context_limit = 2000000

        compaction_active = bool(virtual_tokens > physical_context_limit)

        return web.json_response({
            "session_id": session_id,
            "physical_tokens": physical_tokens,
            "virtual_tokens": virtual_tokens,
            "physical_context_limit": physical_context_limit,
            "virtual_context_limit": virtual_context_limit,
        ...
        *route* is an optional ``model_routes`` entry (resolved from the
        ...
        def _run():
            ...
            try:
                ...
                if comp and hasattr(comp, "get_virtual_tokens"):
                    input_tokens = comp.get_virtual_tokens(input_tokens)
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            self._app.router.add_get("/api/session/context", self._handle_session_context)
