...
def _finalize_session(session: dict | None, end_reason: str = "tui_close") -> None:
    ...
    # ── Auto-compress large idle context on teardown to flush KV-cache to RAG ──
    if agent is not None and history:
        try:
            from agent.conversation_compression import compress_context
            from agent.model_metadata import estimate_messages_tokens_rough
            
            approx_tokens = estimate_messages_tokens_rough(history)
            # Threshold: 15,000 tokens (significant size to justify compression)
...
def _get_usage(agent) -> dict:
    ...
    if comp:
        ...
        if last_prompt < 0:
            last_prompt = 0
        if hasattr(comp, "get_virtual_tokens"):
            last_prompt = comp.get_virtual_tokens(last_prompt)
