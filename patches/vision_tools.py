...
# Monkeypatch ONNX Runtime InferenceSession to limit threads inside container
try:
    import onnxruntime as ort
    _original_init = ort.InferenceSession.__init__
    def _patched_init(self, model_path, sess_options=None, *args, **kwargs):
        if sess_options is None:
            sess_options = ort.SessionOptions()
        sess_options.intra_op_num_threads = 2
        sess_options.inter_op_num_threads = 2
        _original_init(self, model_path, sess_options, *args, **kwargs)
...
async def vision_analyze_tool(
    ...
    try:
        ...
        # Step 1: Run OCR first (to see if we can short-circuit the LLM call)
        ocr_text = ""
        try:
            from rapidocr_onnxruntime import RapidOCR
            ocr_engine = RapidOCR()
            ocr_result, _ = ocr_engine(str(temp_image_path))
            if ocr_result:
                ocr_lines = [item[1] for item in ocr_result]
                ocr_text = "\n".join(ocr_lines)
        except Exception as ocr_err:
            logger.warning("RapidOCR failed to extract text: %s", ocr_err)

        # Step 2: Decide whether to short-circuit
        # If the OCR extracted a significant amount of text (e.g. > 150 characters),
        # we skip the LLM call entirely and return the OCR output.
        if ocr_text and len(ocr_text) > 150:
            logger.info("Adaptive Routing: High text content detected (%d chars). Short-circuiting LLM call.", len(ocr_text))
            result = {
                "success": True,
                "analysis": f"[Text-Heavy Image Ingested via OCR]:\n\n{ocr_text}"
            }
...
VISION_ANALYZE_SCHEMA = {
    "name": "vision_analyze",
    "description": (
        "Load an image into the conversation to inspect it. Accepts a "
        "URL, local file path, or data URL. For non-vision text models, "
        "this delegates the image to an auxiliary vision model which returns "
        "a detailed text description answering your specific question. "
        "Because you will only see the text description returned by this tool "
        "and not the raw pixels, you MUST make the 'question' parameter extremely "
        "detailed, specifying exactly what data, values, or visual features you need "
        "extracted."
    ),
    "parameters": {
        "type": "object",
        "properties": {
            ...
            "question": {
                "type": "string",
                "description": (
                    "Your specific, detailed question or instruction about the image "
                    "(e.g., 'Extract the benchmark scores for model X and Y from the table'). "
                    "Be highly specific, as this is passed directly to the vision model "
                    "to generate the text description you will receive."
                )
            }
        },
