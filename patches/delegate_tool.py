...
def _register_subagent(record: Dict[str, Any]) -> None:
    ...
    if agent:
        ...
        if session_id:
            parent_id = getattr(agent, "parent_session_id", None)
            role = getattr(agent, "_delegate_role", "leaf")
            workspace_mode = getattr(agent, "_workspace_mode", "inherit")
            goal = record.get("goal")
            model = record.get("model")
            c_used = getattr(agent, "session_prompt_tokens", 0) + getattr(agent, "session_completion_tokens", 0)
            c_max = int(getattr(agent.context_compressor, "context_length", 0) or 0) if getattr(agent, "context_compressor", None) else 0
            try:
                from agent.context_breakdown import update_swarm_session
...
def _build_child_agent(
    ...
    child._workspace_mode = workspace_mode
    ...
    def _heartbeat_loop():
        while not _heartbeat_stop.wait(_HEARTBEAT_INTERVAL):
            ...
            try:
                ...
                pass
            # Update swarm registry dynamically during heartbeat
            try:
                from agent.context_breakdown import update_swarm_session
                c_used = getattr(child, "session_prompt_tokens", 0) + getattr(child, "session_completion_tokens", 0)
                c_max = int(getattr(child.context_compressor, "context_length", 0) or 0) if getattr(child, "context_compressor", None) else 0
                update_swarm_session(
                    session_id=child.session_id,
                    role=getattr(child, "_delegate_role", "leaf"),
                    workspace_mode=getattr(child, "_workspace_mode", "inherit"),
                    active=True,
                    parent_uuid=getattr(child, "parent_session_id", None),
                    goal=goal,
...
def delegate_task(
    ...
    top_workspace_mode = workspace_mode or "inherit"
    ...
    max_spawn = min(3, _get_max_spawn_depth())  # Hard swarm delegation nesting depth limit
    if depth >= max_spawn:
        return json.dumps(
            {
                "error": (
                    f"Delegation depth limit reached (depth={depth}, "
                    f"max_spawn_depth={max_spawn}). Swarm nesting is limited to a maximum depth of 3 "
                    f"to prevent recursive subagent runaways."
                )
            }
        )
    ...
    if tasks and isinstance(tasks, list):
        ...
        task_list = [{"goal": goal, "context": context, "role": top_role, "workspace_mode": top_workspace_mode}]
    else:
    ...
    try:
        for i, t in enumerate(task_list):
            ...
            effective_workspace_mode = t.get("workspace_mode") or top_workspace_mode
    ...
    "parameters": {
        ...
        "properties": {
                ...
                "items": {
                    ...
                    "properties": {
                        ...
                        "workspace_mode": {
                            ...
                            "description": "Per-task workspace mode override. See top-level 'workspace_mode' for semantics.",
                        },
                    },
                    "required": ["goal"],
                },
            ...
            "workspace_mode": {
                "type": "string",
                "enum": ["inherit", "share", "branch"],
                "description": "Workspace mode for child agents ('inherit', 'share', 'branch').",
            },
            "background": {
                "type": "boolean",
                "description": (
